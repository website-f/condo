(function ($) {
  ...
  $("#install-activate-button").on("click", function (e) {
    ...
    var activationData = {
      action: "wydegrid_install_and_activate_plugins",
      nonce: wydegrid_localize.nonce,
    };
    $.post(wydegrid_localize.ajax_url, activationData, function (response) {
      ...
      if (match) {
        ...
        try {
          ...
          if (responseObj.success === true) {
            window.location.href = wydegrid_localize.redirect_url;
          } else {
            console.log("Error!");
          }
        } catch (error) {
  ...
  $("#wydegrid-dismiss-notice").on("click", ".notice-dismiss", function () {
    $.ajax({
      url: ajaxurl,
      data: {
        action: "wydegrid_dashboard_dismissble_notice",
        nonce: wydegrid_localize.nonce,
      },
    });
  });
