...
namespace FSPoster\GuzzleHttp\Handler;

use FSPoster\GuzzleHttp\Exception\ConnectException;
use FSPoster\GuzzleHttp\Exception\RequestException;
use FSPoster\GuzzleHttp\Promise as P;
use FSPoster\GuzzleHttp\Promise\FulfilledPromise;
use FSPoster\GuzzleHttp\Promise\PromiseInterface;
use FSPoster\GuzzleHttp\Psr7\LazyOpenStream;
use FSPoster\GuzzleHttp\TransferStats;
use FSPoster\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \FSPoster\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \FSPoster\GuzzleHttp\Psr7\Utils::streamFor($sink);
