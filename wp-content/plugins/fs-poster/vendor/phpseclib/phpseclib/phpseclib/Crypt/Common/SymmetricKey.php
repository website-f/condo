 ...
 */
namespace FSPoster\phpseclib3\Crypt\Common;

use FSPoster\phpseclib3\Common\Functions\Strings;
use FSPoster\phpseclib3\Crypt\Blowfish;
use FSPoster\phpseclib3\Crypt\Hash;
use FSPoster\phpseclib3\Exception\BadDecryptionException;
use FSPoster\phpseclib3\Exception\BadModeException;
use FSPoster\phpseclib3\Exception\InconsistentSetupException;
use FSPoster\phpseclib3\Exception\InsufficientSetupException;
use FSPoster\phpseclib3\Exception\UnsupportedAlgorithmException;
use FSPoster\phpseclib3\Math\BigInteger;
use FSPoster\phpseclib3\Math\BinaryField;
use FSPoster\phpseclib3\Math\PrimeField;
/**
 * Base Class for all \phpseclib3\Crypt\* cipher classes
 *
 * @author  Jim Wigginton <terrafrost@php.net>
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \\FSPoster\\phpseclib3\\Common\\Functions\\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \\FSPoster\\phpseclib3\\Common\\Functions\\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \\FSPoster\\phpseclib3\\Common\\Functions\\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
