...
use FSPoster\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
use FSPoster\phpseclib3\Crypt\EC\BaseCurves\Prime as PrimeCurve;
...
trait Common
{
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\\FSPoster\\phpseclib3\\Crypt\\EC\\Curves\\' . $params['namedCurve'];
    ...
    private static function encodeParameters(BaseCurve $curve, $returnArray = \false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            ...
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'FSPoster\\phpseclib3\\Crypt\\EC\\Curves\\' . $testName;
