...
class PHPImage
{
	...
	public function rectangle (
		...
		if ( $outline === true )
		{
			imagerectangle( $this->img, $x, $y, $x + $width, $y + $height, imagecolorallocatealpha( $this->img, $colour[ 0 ], $colour[ 1 ], $colour[ 2 ], intval(( 1 - $opacity ) * 127 )) );
		}
		else
		{
			imagefilledrectangle( $this->img, $x, $y, $x + $width, $y + $height, imagecolorallocatealpha( $this->img, $colour[ 0 ], $colour[ 1 ], $colour[ 2 ], intval(( 1 - $opacity ) * 127 )) );
		}
		$this->afterUpdate();
	...
	public function draw ( $file, $x = '50%', $y = '50%', $wr = null, $hr = null )
	{
		if ( $info = $this->getImageInfo( $file ) )
		{
			...
			// Draw image
			imagecopyresampled( $this->img, $image, intval($x + ( $width - $wr ) / 2), intval($y + ( $height - $hr ) / 2), 0, 0, $wr, $hr, $width, $height );
			imagedestroy( $image );
			$this->afterUpdate();
	...
	public function text ( $text, $options = [] )
	{
        ...
        $maxLineHeight = intval($fontHeight * $this->lineHeight);

        $firstLineBbox = imageftbbox($fontSize, $angle, $fontFile, $lines[0]);
        $firstLineYCoordinates = [$firstLineBbox[1], $firstLineBbox[3], $firstLineBbox[5], $firstLineBbox[7]];
        $firstLineMinY = min($firstLineYCoordinates);

        if ( $drawBackground && ! empty( $backgroundColor ) && ! empty( $backgroundOpacity ) && is_int( $width ) )
        {
            $textPadding = 10;

            $this->rectangle( $x, $y + $firstLineMinY - $textPadding, $width, 2 * $textPadding + $maxLineHeight * count($lines), $backgroundColor, $backgroundOpacity );
        }

		foreach ( $lines as $index => $line )
		{
            $bbox = imageftbbox($fontSize, $angle, $fontFile, $line);

            $xCoordinates = [$bbox[0], $bbox[2], $bbox[4], $bbox[6]];
            $yCoordinates = [$bbox[1], $bbox[3], $bbox[5], $bbox[7]];
			...
			if ( is_int( $width ) )
			{
				...
				{
					case 'center':
						$offsetx += intval( ( $width - $actualWidth ) / 2 );
						break;
					case 'right':
				...
				if ( $debug )
				{
					$this->rectangle( $x + $minX + $offsetx, $lineY + $minY + $offsety , $actualWidth, $actualHeight, [
						rand( 150, 255 ),
						rand( 150, 255 ),
						rand( 150, 255 ),
					], 0.5 );
			...
			if ( $strokeWidth > 0 )
			{
				...
				for ( $sx = ( $x - abs( $strokeWidth ) ); $sx <= ( $x + abs( $strokeWidth ) ); $sx++ )
				{
					...
					{
						imagefttext( $this->img, (int)$fontSize, (int)$angle, $sx + $offsetx, $sy + $offsety, $tmpStrokeColor, $fontFile, $line );
					}
				}
			}
			// Draw text
