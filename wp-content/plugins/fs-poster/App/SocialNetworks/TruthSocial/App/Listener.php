...
use FSPoster\App\SocialNetworks\TruthSocial\Adapters\PostingDataAdapter;
...
class Listener
{
	...
	public static function getCustomPostData( array $customPostData, Collection $channel, string $socialNetwork )
	{
		...
		$customPostData["attach_link"] = (bool)Settings::get( 'truthsocial_attach_link', true );
		$customPostData["upload_media"] = (bool)Settings::get( 'truthsocial_upload_media', false );
		$customPostData["upload_media_type"] = Settings::get( 'truthsocial_media_type_to_upload', 'featured_image' );

		if( ! empty( $channelSettings[ 'use_custom_post_content' ] ) )
			$customPostData['post_content'] = $channelSettings[ 'post_content' ];
		else
			$customPostData['post_content'] = Settings::get( 'truthsocial_post_content', '{post_title}' );
    ...
    public static function getPostLink ( string $postLink, ScheduleObject $scheduleObj ): string
    {
        ...
        return Api::SERVER . '/@' . $scheduleObj->getChannel()->data_obj->username . '/' . $scheduleObj->getSchedule()->remote_post_id;
    }
    ...
    public static function getChannelLink ( string $channelLink, string $socialNetwork, Collection $channel ): string
    {
        ...
        return Api::SERVER . '/@' . $channel->data_obj->username;
    }
