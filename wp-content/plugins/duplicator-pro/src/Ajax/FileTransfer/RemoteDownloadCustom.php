...
class RemoteDownloadCustom
{
    ...
    public static function dropboxRemoteUrlFilter($downloadData)
    {
        ...
        if (SnapURL::wwwRemove($parseUrl['host']) === 'dropbox.com') {
            ...
            if (isset($queryVals['dl'])) {
                ...
                DupLog::trace("Real Dropbox URL: $realURL");
            ...
            if ($filename && preg_match('/\.(zip|daf)$/i', $filename)) {
                $downloadData['archiveName'] = $filename;
                DupLog::trace("Archive name from Dropbox path: $filename");
            }
        }
        return $downloadData;
    }
    ...
    public static function gDriveRemoteUrlFilter($downloadData)
    {
        ...
        if (SnapURL::wwwRemove($parseUrl['host']) === 'drive.google.com') {
            ...
            DupLog::trace("Real GDrive URL: $indirectURL");
            ...
            if (
                ...
                // Check if this is a virus scan warning for large files
                if ($response->success && stripos($response->body, 'virus scan warning') !== false) {
                    // Extract UUID and filename from virus scan warning page
                    if (preg_match('/<input[^>]*name="uuid"[^>]*value="([^"]*)"/', $response->body, $uuidMatches)) {
                        $uuid = $uuidMatches[1];

                        // Build new URL with UUID
                        $confirmURL = "https://drive.usercontent.google.com/download?" . http_build_query([
                            'id'      => $id,
                            'export'  => 'download',
                            'confirm' => 't',
                            'uuid'    => $uuid,
                        ...
                        $downloadData['url'] = $confirmURL;

                        // Extract filename from the HTML page
                        if (preg_match('/<a href="[^"]*">([^<]+\.(?:zip|daf))<\/a>/', $response->body, $fileMatches)) {
                            $downloadData['archiveName'] = $fileMatches[1];
                            DupLog::trace("Archive name from GDrive virus warning: " . $fileMatches[1]);
            ...
            DupLog::trace("Archive name on GDrive: $archiveName");
            ...
            $_SESSION["duplicator_import_from_link_cookies"] = property_exists($response, 'cookies') ? $response->cookies : null;
        }
    ...
    public static function oneDriveRemoteUrlFilter($downloadData)
    {
        ...
        if (SnapURL::wwwRemove($parseUrl['host']) === '1drv.ms') {
            ...
            DupLog::trace("Next step OneDrive URL: $nextURL");
            ...
            $archiveName                 = $matches[1];
            $downloadData['archiveName'] = $archiveName;
            DupLog::trace("Archive name on OneDrive: $archiveName");

            $realURL = $response->headers["content-location"];
            DupLog::trace("Real OneDrive URL: $realURL");
            $downloadData['url'] = $realURL;
