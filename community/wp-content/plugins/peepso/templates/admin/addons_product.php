...
if (in_array($item_id, $can_install)) {
    ...
    if($item_id==115220226){
        $is_theme = TRUE;
        $is_installed = $block_theme_installed;
        $activation_key = 'peepso-block-theme';
        $is_active = PeepSo3_Utility_Addon_Class::is_active('blocktheme');
    }
    elseif ($item_id == 7354103) {
        // if Gecko
        $is_theme = TRUE;
        ...
        if (count($peepso_plugins) > 0) {
            foreach ($peepso_plugins as $key=> $plugin) {
                ...
                $plugin_name = trim(str_ireplace('(BETA)', '', $plugin_name));
...
//                    echo $key . ' - ' . $plugin_name . ' - ' . $product_title . '<br/>';
                }
            }
        }
    }
    ...
    <div class="pa-addons__addon-header">
        ...
        <div class="pa-addons__addon-actions">
            ...
            if ($can_install_item) {
                ...
                $upgrade_href = 'https://peepso.com/profile/?*/edd/licenses/';
                if(!PeepSo3_Helper_Addons::get_license() || PeepSo3_Helper_Addons::license_is_free_bundle()) {
                    $upgrade_href = 'https://peepso.com/pricing/ref/429';
                }
                ...
                if(in_array(64823085, $bundles)) {
                    ...
                    $page = isset($license_page_reference) ? $license_page_reference : 'peepso-installer';
                    if (isset($license_page_reference) && $license_page_reference == 'peepso-getting-started') {
                        $upgrade_href = admin_url('admin.php?page=' . $license_page_reference . '&section=peepso&step=2&action=peepso-free');
                    } else {
                        $upgrade_href = admin_url('admin.php?page=peepso-installer&action=peepso-free');
                    }
                    $upgrade_target = '';

                    $upgrade_style= "-webkit-linear-gradient(-45deg, #8f6B29, #FDE08D, #DF9F28);background: linear-gradient(-45deg, #8f6B29, #FDE08D, #DF9F28); background-size: 200% 200%; color: #fff !important; text-shadow: 1px 1px 1px rgba(0,0,0, .2);animation: ps-installer-gradient-$item_id 5s ease infinite; box-shadow:none !important;";
                }
                ?>
