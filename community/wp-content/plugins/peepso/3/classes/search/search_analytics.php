...
class PeepSo3_Search_Analytics {
    ...
    public function store($search, $class, $user_id = null) {
    ...
    public function get_recent_searches($limit = 10, $user_id=null) {
        if(!$user_id) {
            $user_id = (int) get_current_user_id();
        }
        if (!$user_id) {
        ...
        // Sanitize the limit to ensure it's a valid number
        $limit = max(1, (int) $limit);

        // Query to get unique recent searches for the user
        $sql = $wpdb->prepare(
            "SELECT DISTINCT search, MAX(id) as max_id 
            FROM {$this->table} 
            WHERE user_id = %d 
            GROUP BY search 
        ...
        // Convert results into a flat array of search terms
        $flat_results = array_map(function($row) {
            return $row['search'];
        }, $results);
