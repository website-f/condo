...
class PeepSoAdmin
{
    ...
    const PEEPSO_URL = 'https://peepso.com';
...
//              $notice .= ' <a href="#TB_inline?inlineId=assign-roles-modal-id" class="thickbox">' . __('Click here', 'peepso-core') . '</a>' . __(' for more information on assigning roles.', 'peepso-core');
    ...
    public function admin_menu()
    {
        ...
        $dashboard_hookname = add_menu_page(__('PeepSo', 'peepso-core'), __('PeepSo', 'peepso-core').$installer_new,
            ...
            PeepSo::get_asset('images/admin/logo-icon_20x20.svg'),
            4);
...
//	}
    ...
    public function enqueue_scripts()
    {
        ...
        if (apply_filters('peepso_free_bundle_should_brand', FALSE) && PeepSo3_Helper_Addons::license_is_free_bundle()) {
            ...
            if(NULL === $pscc) {
                $free_bundle_branding = '';
            } else {
                $free_bundle_branding = $pscc;
            }
        }
...
//        $wp_styles->add_data('ace-admin-ie', 'conditional', 'IE 7');
        ...
        wp_register_style('peepso-started', PeepSo::get_asset('css/started.css'), NULL, PeepSo::PLUGIN_VERSION, 'all');
        ...
        $data = apply_filters('peepso_data', array(
            ...
            'dialog' => $data_dialog,
            'nonce' => wp_create_nonce('peepso-nonce'),
    ...
    public function get_tabs()
    {
        if (NULL === $this->dashboard_tabs) {
            ...
            $show_getting_started = isset($_REQUEST['show_getting_started']) ? intval($_REQUEST['show_getting_started']) : PeepSo::get_option('show_getting_started', 1);
            if(PeepSoSystemRequirements::is_demo_site()) $show_getting_started = 0;
            ...
            // #6618 PFB CTA
            if($d=PeepSo3_Helper_Addons::license_is_eligible_upgrade()) {
                $tabs['red']['upgrade'] = array(
                    'slug' => 'https://peepso.com/Pricing/ref/429/',
                    'menu' => '<span class="wp-submenu_upgrade-peepso"><i class="gcis gci-gem"></i> <span>' . __('Upgrade Now!', 'peepso-core') . "<!--$d-->".'</span></span>',
                    'icon' => 'admin-plugins',
                    // 'function' => array('PeepSoAdmin', 'addons'),
                    //'count' => __('NEW!', 'peepso-core'),
                );
    ...
    public static function perform_activation($category)
    {
        ...
        if( is_array($addons) && count($addons) ) {
            ...
            if(strlen($debug)) { ?>
                <div class='error peepso is-dismissible pa-activation-error'>
                    ...
                    <b>Need assistance? We are here to help! <a class="ps-emphasis" href="https://peepso.com/contact" target="_blank">Contact PeepSo</b></a>.
                    <br/>
                    <ul>
    ...
    {
        $ret = array(
            'member'    => __('Community Member', 'peepso-core'),
            'moderator' => __('Community Moderator', 'peepso-core'),
            'admin'     => __('Community Administrator', 'peepso-core'),
            'ban'       => __('Banned', 'peepso-core'),
            'register'  => __('Pending user email verification', 'peepso-core'),
            'verified'  => __('Pending admin approval', 'peepso-core'),
            #'user'         => __('role_user', 'peepso-core'),
    ...
    public function enqueue_ace_admin_scripts()
    {
        ...
        wp_enqueue_style('peepso-started');
    }
        ...
        <div class="row">
            ...
            if(!isset($_GET['newsletter_test'])) {
                ...
                if (empty($newsletter_form)) {
                    $url = 'peepsotools-integration-json/newsletter.txt';
                    $pscc =  /** checked for NULL**/ (new PeepSoCom_Connect($url))->get();

                    if (NULL === $pscc) {
                        $newsletter_form = '';
                    } else {
                        $newsletter_form = $pscc;
                ...
                if (!strlen($newsletter_form)) {
                    $url = 'peepsotools-integration-json/newsletter_new.txt';
                    $pscc =  /** checked for NULL**/ (new PeepSoCom_Connect($url))->get();
    ...
    public function recent_posts_tab()
    {
        ...
        if (0 === $posts->post_count) {
            ...
            foreach ($posts->posts as $post) {
                ...
                $dashboard_content = apply_filters('peepso_dashboard_most_recent_content', $post->ID, $post->post_content);
                ...
                echo '          <span class="green">', esc_attr(PeepSoTemplate::time_elapsed(strtotime($post->post_date_gmt) + get_option('gmt_offset') * HOUR_IN_SECONDS, current_time('timestamp'))), ' </span>', PHP_EOL;
                echo '      </div>', PHP_EOL;
                ...
                echo '      </div>', PHP_EOL;
                echo '      <div class="text">', esc_attr(ucfirst($type->labels->activity_action)), ': "', wp_kses_post(substr(strip_tags(apply_filters('filter_remove_location_shortcode', $dashboard_content)), 0, 30)), '"', PHP_EOL;
                echo '      </div>', PHP_EOL;
        ...
        if ($pagenow == 'admin.php' && isset($_GET['page']) && $_GET['page'] == 'peepso') {
            ...
            if (!empty($post) && !empty($post['register_nonce'])) {
                ...
                if ($nonceCheck) {
                    ...
                    $response	 = wp_remote_post($href, $args); # Not a critical PeepSo.com call, DON'T use PeepSoCom_Connect()
            ...
            {
                $dashboard_main = __('Once registered, you will receive updates and special offers from PeepSo. We will send your administrator\'s email and site URL to PeepSo server.', 'peepso-core');
