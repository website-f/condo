...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		/* Page Content  */
		if ( empty( $import_file_info['import_page_file_url'] ) ) {
			if ( ! empty( $import_file_info['local_import_page_file'] ) && file_exists( $import_file_info['local_import_page_file'] ) ) {
				$downloaded_files['page-content'] = $import_file_info['local_import_page_file'];
			}
		} else {		
			
			// Set the filename string for content import file.
			$content_filename = self::apply_filters( 'ocdi/downloaded_content_page_file_prefix', 'demo-content-page-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'ocdi/downloaded_content_page_file_suffix_and_file_extension', '.xml' );

			// Download the content import file.
			$downloaded_files['page-content'] = $downloader->download_file( $import_file_info['import_page_file_url'], $content_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['page-content'] ) ) {
				return $downloaded_files['page-content'];
			}
		}
		
		/* Products Content */
		if ( empty( $import_file_info['import_products_file_url'] ) ) {
			if ( ! empty( $import_file_info['local_import_products_file'] ) && file_exists( $import_file_info['local_import_products_file'] ) ) {
				$downloaded_files['products-content'] = $import_file_info['local_import_products_file'];
			}
		} else {			
			
			// Set the filename string for content import file.
			$content_filename = self::apply_filters( 'ocdi/downloaded_content_products_file_prefix', 'demo-content-products-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'ocdi/downloaded_content_products_file_suffix_and_file_extension', '.xml' );

			// Download the content import file.
			$downloaded_files['products-content'] = $downloader->download_file( $import_file_info['import_products_file_url'], $content_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['products-content'] ) ) {
				return $downloaded_files['products-content'];
			}
